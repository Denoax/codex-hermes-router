...
IRIS_SKILL_DST="${HOME}/.agents/skills/iris-camera"
...
for p in "$BIN_DST" "$SKILL_DST" "$IRIS_SKILL_DST" "$PLUGIN_DST"; do
  [[ ! -e "$p" ]] || { echo "ERROR: refusing to overwrite existing path: $p" >&2; exit 1; }
done
printf 'Will install:\n  %s\n  %s\n  %s\n  %s\n' "$BIN_DST" "$SKILL_DST" "$IRIS_SKILL_DST" "$PLUGIN_DST"
...
cp -a "$ROOT/skills/iris-camera" "$IRIS_SKILL_DST"
